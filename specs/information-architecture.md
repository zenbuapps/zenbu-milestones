...
│   │   └── 有 roadmap 的 repo × N（字母序）
│   │         - 左：lock icon（if private）+ repo name
│   │         - 右：roadmapCount badge
│   └── "其他 repos（無 roadmap）" collapse section
│         └── 無 roadmap 的 repo × M（外連到 GitHub，不進 SPA）
│
...
**用途**：一眼看到整個 org 的 roadmap 進度。

### 區塊（由上至下）

#### 1. PageHeader
- 標題「總覽」
- 描述「所有專案的 roadmap 進度與 roadmap」
...
| #2 | 進行中 Roadmaps | `totals.openRoadmaps` | `已完成 N` | `Clock` | blue-600 |
| #3 | 逾期 Roadmaps | `totals.overdueRoadmaps` | 無 | `AlertTriangle` | orange-500（若 > 0 值為紅字）|
...
- 只看 `roadmapCount > 0` 的 repo

**B. Roadmap 狀態分布（StatusDonutChart）**
...
- description（最多 2 行）
- 進度列（`closedRoadmapCount / roadmapCount`）+ `completionRate` 百分比
- 「下一個 roadmap」區塊（若 `nextDueRoadmap` 不為 null）
- 「N 個 roadmap 逾期」警示（若 `overdueCount > 0`）
- 底部：「查看 Roadmap」（SPA 內連）+ "GitHub"（外連）

**空狀態**：若沒有任何 active repo → `EmptyState` + `Inbox` 圖示 + 提示「當 org 底下的 repo 建立了 roadmap 後，會自動出現在這裡」

---

## RoadmapPage（`#/repo/:name`）

**用途**：看單一 repo 的 roadmap 時間線與其下 issue 完成狀態。

### 載入狀態
- 載入中 → `LoadingSpinner size="lg"` 置中
- 錯誤（通常是 404：該 repo 無 roadmap，沒有 detail 檔）→ `EmptyState` + 返回按鈕
- 空（該 repo 無 roadmap，但 detail 檔竟存在）→ `EmptyState` + 提示建立 roadmap
...
| 總 Roadmaps | `detail.roadmaps.length` |
| 完成率 | `Math.round(closedRoadmapCount / total * 100)%` |

#### 4. RoadmapTimeline
垂直時間軸。每個節點 `RoadmapNode`：
- 左側狀態圓點（依 `deriveRoadmapStatus` 四色）
- 右側卡片：roadmap 標題（外連 GitHub）+ StatusBadge + 日期文字 + description + ProgressBar + 展開按鈕
- **預設展開**：`pickDefaultExpanded` 優先序 = 最近 in_progress > 第一個 overdue > 第一個
- 展開後顯示 `IssueList`（issue 標題、labels、assignees 頭像）

### 排序規則
- 有 `dueOn` 的 roadmap 依 `dueOn` 升序
...
4. 若有逾期 → 從 Sidebar 或 RepoCard 的警示進去看具體哪些 roadmap
...
3. 展開 in_progress roadmap 看 issue 細節
4. 點 issue 外連到 GitHub 處理

### Journey 3：新 repo 加入組織
1. CI 下一次整點 cron 會重抓、包含新 repo
2. Sidebar 自動出現新 repo（若有 roadmap → 主列表；若無 → 收摺區）
3. 使用者無需手動動作
