...
> **過渡期狀態（2026-04-21 起）**：舊 GitHub Pages 部署與 cron fetch workflow 已退役；資料源目前雙軌並存（舊靜態 JSON 管線 + 新後端 API），前端部署平台遷移計畫待定。
...
- 觸發關鍵字：`fetch-data.ts`、`loader.ts`、`types.ts`、`api.ts`、`AppShell`、`OverviewPage`、`RoadmapPage`、`HashRouter`、`BASE_URL`、`@prisma/client`、`DATABASE_URL`、`VITE_API_BASE_URL`
...
```
[GitHub REST API]
       ↓ (本地執行 GH_TOKEN=xxx pnpm fetch-data)
...
[apps/web/public/data/]
   ...
   ├── repos.json
   └── repos/{name}.json
       ↓ (runtime, apps/web/src/data/loader.ts)
   fetch() + Map cache + import.meta.env.BASE_URL
       ↓
[React SPA]
   AppShell（載 summary）/ OverviewPage / RoadmapPage
```

**新管線**：
```
[瀏覽器]
   apps/web/src/data/api.ts （fetch VITE_API_BASE_URL + credentials: include）
       ↓
[NestJS apps/api]
   Auth (Google OAuth) / Issues / Admin / Uploads / Repos
       ↓
[PostgreSQL via Prisma]
```

目前 `/api/summary` 與 `/api/repos/:name/detail` **尚未實作**；`AppShell` / `RoadmapPage` 仍依賴舊管線。

### 共用契約：`apps/web/src/data/types.ts`

fetcher、loader、SPA **共用** 這一份型別定義。**任何欄位變動都是跨端契約變動**，依 `.claude/rules/data-contract.rule.md` 的流程處理。
...
| 任務類型 | 讀取 rule |
|---|---|
| 新增 / 修改 `apps/web/src/data/types.ts` 的欄位；改 `fetch-data.ts` 的產出形狀 | `.claude/rules/data-contract.rule.md` |
...
| 場景 | Skill |
|---|---|
| 程式 import 自 `@octokit/rest` 或動 `apps/web/scripts/fetch-data.ts` / 後端 GitHub service | `.claude/skills/octokit-rest-v21/SKILL.md` |
...
1. **我是否知道這個變更會跨越 fetcher / loader / SPA / API 多端？** 若是，必讀 `data-contract.rule.md`
...
6. **我是否使用一個版本敏感的 library API？** 若是，載對應 lib skill
...
### 「加一個新指標到 StatCard」
1. 確認資料是否已在 `Summary.totals`（`apps/web/src/data/types.ts`）
2. 若無 → 走 `data-contract.rule.md` 的「新增欄位流程」，同時改 `apps/web/scripts/fetch-data.ts`
3. 在 `apps/web/src/pages/OverviewPage.tsx` 加一張 `<StatCard>`（用既有 `lucide-react` 圖示）
4. `pnpm typecheck` → `pnpm build` → `pnpm preview` 驗證

### 「新增一個敏感 label」
1. 改 `apps/web/scripts/fetch-data.ts` 的 `SENSITIVE_LABELS` 集合
...
### 「加一個新的後端 endpoint」
1. 讀 `packages/shared` 看 DTO 有沒有相對應；若無，在 shared 新增並 build
...
4. 前端在 `apps/web/src/data/api.ts` 加對應函式
...
- 動兩階段管線（退役舊管線 / 補新 API endpoint）→ 改 `data-contract.rule.md` + CLAUDE.md + 本 skill
