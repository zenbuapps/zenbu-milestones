...
> **狀態**：舊 GitHub Pages 靜態部署已於 2026-04-21 退役，舊靜態 fetcher (`apps/web/scripts/fetch-data.ts` + `public/data/*.json`) 也一併移除。資料現由後端 `DashboardModule` runtime 抓 GitHub + 5 分鐘 in-memory TTL cache 提供。前端部署平台遷移計畫待定（候選：Vercel / Cloudflare Pages）。
...
⚠️ 任何 secret **絕對不可有 `VITE_` 前綴**，否則會被打包進前端 bundle 外洩。
...
`apps/web/src/App.tsx` 使用 `HashRouter`（舊 GitHub Pages 部署遺留）。兩條路由：

- `/` → `OverviewPage`（所有 repo）
- `#/repo/:name` → `RoadmapPage`（單一 repo 的 roadmap / issue）

**兩頁都需登入**才能瀏覽。未登入時 `AppShell` / `RoadmapPage` 會掛 `<RequireAuthGate />`（全螢幕登入提示 + Google 登入按鈕）。投稿 / admin 流程沿用 auth 邏輯。
